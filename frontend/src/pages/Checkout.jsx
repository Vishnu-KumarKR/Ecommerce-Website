  ...
  const handlePlaceOrder = async (e) => {
    ...
    try {
      const payload = {
        ...
        total: getCartTotal(),
        address: address,
        // paymentMethod: selectedPayment // Backend doesn't store this yet but it's fine
      };

      const res = await apiClient.post('/api/orders', payload);

      if (res.data.success) {
        setPlacedOrderId(res.data.orderId); // Use ID from backend if available, or just a placeholder
        setShowSuccess(true);
      } else {
