...
// activeTokens Map removed - using DB sessions now
...
app.post('/api/orders', authenticate, (req, res) => {
  ...
  try {
    const info = db.prepare(`
      INSERT INTO orders (user_id, items, total, status, address, date)
      VALUES (@user_id, @items, @total, @status, @address, @date)
    `).run({
      user_id: req.user.id,
    ...
    res.status(201).json({ success: true, orderId: info.lastInsertRowid });
  } catch (err) {
    console.error(err);
...
app.get('/api/profile', authenticate, (req, res) => {
  try {
    ...
    if (!row) {
      // Return basic info from user record if no profile exists yet
      return res.json({
    ...
    res.json({
      ...
      phone: row.phone || '',
      gender: row.gender || '',
      address: row.address ? JSON.parse(row.address) : { line1: '', city: '', state: '', zip: '' }
    });
  } catch (err) {
...
app.put('/api/profile', authenticate, (req, res) => {
  ...
  try {
    const exists = db.prepare('SELECT 1 FROM profiles WHERE user_id = ?').get(req.user.id);
    if (exists) {
      db.prepare(`
        UPDATE profiles 
        SET fullName = @fullName, phone = @phone, gender = @gender, address = @address
      ...
      db.prepare(`
        INSERT INTO profiles (user_id, fullName, phone, gender, address)
        VALUES (@user_id, @fullName, @phone, @gender, @address)
      `).run({
        user_id: req.user.id,
        fullName,
        phone,
